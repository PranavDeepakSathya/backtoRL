...
class CEnv:
  def __init__(self, n, num_envs, checker_id=CHECKER_C4):
    ...
    # obs is now upper-triangle only, stored as bool (0/1 uint8 from C)
    self.obs = np.frombuffer(
        (ctypes.c_uint8 * (num_envs * self.num_actions)).from_address(
            ctypes.addressof(obs_raw.contents)), dtype=np.bool_
    ).reshape(num_envs, self.num_actions)
    ...
    # precompute upper triangle edge index → (u, v) for action dispatch
  ...
  def step(self, actions):
    ...
    Returns: (obs, reward, done) — obs is bool array (num_envs, num_actions).
    """
  ...
  def reset(self):
    """Returns obs as bool array (num_envs, num_actions)."""
