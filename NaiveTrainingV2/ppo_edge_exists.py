"""
PPO driver for EdgeTransformerPolicyExists.
Same as ppo_edge.py but uses the exists-only feature policy
(proj_in = Linear(1, d) — no degree or common-neighbour cheating).
"""
...
def train(cfg=CFG, resume=None):
    ...
    policy = EdgeTransformerPolicyExists(
    ...
    print(f"\nPPO (exists-only)  n={cfg['n']}  envs={cfg['num_envs']}  checker={cfg['checker_id']}  device={cfg['device']}")
...
def train_multi_n_random(train_ns, policy, optimizer, cfg, n_cycles=3, global_step=0):
    ...
    Identical to ppo_edge.train_multi_n_random but uses EdgeTransformerPolicyExists.
    """
    device         = torch.device(cfg['device'])
    steps_per_iter = cfg['n_steps'] * cfg['num_envs']
