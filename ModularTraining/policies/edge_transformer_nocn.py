...
class _Block(nn.Module):
    """Pre-norm transformer block, 2x FFN. bias is (E, E) — static, broadcast over B and heads."""
...
class EdgeTransformerNoCNPolicy(nn.Module):
    """
    Edge-token transformer policy for Turan RL — no common-neighbours feature.

    Each of the E = n(n-1)/2 edge slots is a sequence token.
    Features per token: [edge_exists, deg_i/(n-1), deg_j/(n-1)]
    ...
    n is inferred from obs.shape[-1] at every call; proj_in is Linear(3, d) so
    ...
    def forward(self, obs):
        ...
        feats = torch.stack([x, deg[:, ui], deg[:, vi]], dim=-1)
