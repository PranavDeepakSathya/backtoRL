...
def collect_rollout(env, policy, n_steps, device):
    ...
    obs = torch.tensor(env.obs.astype(np.float32), device=device)
    for t in range(n_steps):
        with torch.no_grad():
            action, log_prob, _, value = policy.get_action(obs)
        next_obs, reward, done = env.step(action.cpu().numpy().astype(np.int32))
        obs_buf[t]  = obs
        act_buf[t]  = action
        rew_buf[t]  = torch.tensor(reward.copy(), device=device)
...
def load_checkpoint(path, policy, device=None):
...
def train(cfg, policy, resume=None):
    ...
    if resume:
        policy, optimizer, cfg, start_iter, global_step, best_mean_ret = \
            load_checkpoint(resume, policy, device=str(device))
        start_iter += 1
