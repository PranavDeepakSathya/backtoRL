...
Env* env_create(int n, int num_envs) {
  ...
  env->rows     = rows;
  env->obs        = (uint8_t*)calloc(num_envs * n * n, sizeof(uint8_t));
...
void env_reset_single(Env* env, int e) {
  int n    = env->n;
  int rows = env->rows;
  memset(env->obs    + e * n * n,    0, n * n * sizeof(uint8_t));
...
void env_reset_all(Env* env) {
  ...
  int n        = env->n;
  int rows     = env->rows;
  memset(env->obs,        0, num_envs * n * n    * sizeof(uint8_t));
